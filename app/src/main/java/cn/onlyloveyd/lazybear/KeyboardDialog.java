...
public class KeyboardDialog extends Dialog implements KeyboardView.OnKeyboardActionListener {
    ...
    private static final int ORDER_CLOSE = 3;
    public static final int MAX_PINCODE_LENGTH = 8;
    ...
    private WeakReference<PinCodeEditText> mTargetEditText;
    private KeyboardAttribute attribute;

    public KeyboardDialog(Context context, PinCodeEditText editText) {
    ...
    public static KeyboardDialog show(Context context,PinCodeEditText editText) {
    ...
    private void onCurrentKeyboardChange() {
        ...
        switch (mCurrentOrder) {
            ...
            case ORDER_LETTER:
                ...
                break;
            default:
//                throw new IllegalStateException(getContext().getString(R.string.error));
        }
    }
    ...
    public static DoCallback doCallback;
    @Override
    public void onKey(int primaryCode, int[] keyCodes) {
        Editable editable = mTargetEditText.get().getText();
        int start = mTargetEditText.get().getSelectionStart();
        Log.d("LXP","primaryCode:"+primaryCode);
        if (primaryCode == Keyboard.KEYCODE_CANCEL) {
            hideKeyboard();
            if(doCallback!=null && (PinCodeEditText.textLength == 6 || PinCodeEditText.textLength == 8)){
                doCallback.startCastCallback();
            }
